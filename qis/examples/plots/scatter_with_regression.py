"""
Scatter-plot diagnostics from ``qis.plots.scatter``.

Generates synthetic ``y = beta * x + eps`` data with a heteroscedastic
beta and renders the scatter with regression line and confidence-band
overlays via ``plot_scatter`` and ``plot_classification_scatter``.
"""
import numpy as np
import pandas as pd
