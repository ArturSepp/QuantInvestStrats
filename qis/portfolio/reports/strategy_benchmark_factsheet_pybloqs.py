...
def generate_multi_portfolio_factsheet_with_pybloqs(multi_portfolio_data: MultiPortfolioData,
    ...
    fig_size = get_df_table_size(df=ra_perf_table)
    ...
    for idx, perf_column in enumerate(perf_columns):
        ...
        colors = compute_heatmap_colors(a=df.to_numpy())
    ...
    b_fig_regime = p.Block(
        [p.Paragraph(f"Sharpe in {str(benchmark)} Bear/Normal/Bull {regime_classifier.freq}-freq regimes", **KWARGS_TITLE),
