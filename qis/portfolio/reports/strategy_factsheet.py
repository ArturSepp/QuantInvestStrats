"""
the one-strategy factsheet: a ``PortfolioData`` and its benchmarks in, a list of pages out.

``generate_strategy_factsheet`` is the flagship report and the archetype the other factsheets
follow. It takes a portfolio - simulated by ``qis.backtest_model_portfolio`` or assembled from a
live track record, the report does not care which - plus one or more benchmark series, and
returns one ``plt.Figure`` per page for ``qis.save_figs_to_pdf``.

Page one always renders and is the whole report by default: cumulative performance with regime
shading, the risk-adjusted performance tables, rolling volatility, Sharpe and beta, drawdowns
and time under water, exposures, turnover and costs. Each ``add_*`` flag appends a further page
- current positions and VaR, grouped weights, the current signal state, weight changes,
performance and risk attribution, per-instrument history - and all of them default to False.

The panels do not share one sampling grid, which is why the signature carries a frequency and a
window per panel rather than a single ``freq``. Drawdowns are read on the nav's native path
while betas are read weekly, and every panel prints the frequency it was computed at in its own
title. Setting them by hand is possible but not the intended route:

    **qis.fetch_default_report_kwargs(time_period=time_period)

supplies a mutually consistent set, chosen from the data's reporting frequency and the length of
the reported span. Reaching more than ten instruments without ``is_grouped`` is switched to
grouped with a warning, since the per-instrument tables stop being legible.

Panels come from ``PortfolioData``'s own ``plot_*`` methods and from the ``qis.plots`` layer;
this module only lays them out on a gridspec. Comparing two portfolios is
``strategy_benchmark_factsheet.py``, several is ``multi_strategy_factsheet.py``, and the facade
that picks between them by input shape is ``factsheet_facade.py``.
"""
# packages
import warnings
import pandas as pd
...
def generate_strategy_factsheet(portfolio_data: PortfolioData,
    ...
    validate_reporting_frequency(portfolio_nav, perf_params.freq)
    ...
    # drawdowns / under-water / cumulative are computed directly on joint_prices (the series passed,
    # on its native grid - never resampled to the reporting frequency), so their titles carry the
    # frequency of that series. Inferred from joint_prices itself, robust to holiday gaps.
    nav_freq = infer_data_frequency_label(joint_prices)
    ...
    qis.plot_rolling_time_under_water(prices=joint_prices,
                                      title=f'Running Time under Water{nav_freq_label}',
                                      dd_legend_type=dd_legend_type,
                                      ax=ax, **kwargs)
    qis.add_bnb_regime_shadows(ax=ax, pivot_prices=pivot_prices, regime_classifier=regime_classifier)
    set_spines(ax=ax, bottom_spine=False, left_spine=False)
    ...
    qis.plot_stack(df=exposures,
                   use_bar_plot=True,
                   title=f'Exposures ({weights_freq}-freq)' if weights_freq is not None else 'Exposures',
