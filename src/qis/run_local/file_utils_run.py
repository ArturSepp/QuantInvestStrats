...
def run_local(local: Locals):
    ...
    elif local == Locals.EXCEL_TRANSPOSE_DICT:
    ...
    elif local == Locals.EXCEL_SERIES_COERCION:
        s = _make_df().iloc[:, 0]
    ...
    elif local == Locals.EXCEL_BAD_SHEET_NAMES:
    ...
    elif local == Locals.EXCEL_EMPTY_INPUT_RAISES:
        try:
    ...
    elif local == Locals.EXCEL_MISMATCHED_SHEET_NAMES_RAISES:
        dfs = [_make_df(seed=0), _make_df(seed=1), _make_df(seed=2)]
        try:
    ...
    elif local == Locals.EXCEL_APPEND_MODE:
        df1 = _make_df(seed=0)
        df2 = _make_df(seed=1)
    ...
    elif local == Locals.CSV_ROUNDTRIP:
        df = _make_df()
    ...
    elif local == Locals.CSV_NONE_RAISES:
        try:
            fu.save_df_to_csv(df=None, file_name='t_csv_none', **_save_kwargs())
        except ValueError as e:
            print(f"got expected ValueError: {e}")

    elif local == Locals.FEATHER_ROUNDTRIP:
        df = _make_df()
    ...
    elif local == Locals.FEATHER_APPEND:
        df1 = _make_df(n=5, seed=0)
        df2 = _make_df(n=5, seed=1)
        df2.index = df1.index + pd.Timedelta(days=5)
    ...
    elif local == Locals.FEATHER_DICT:
        dfs = {'one': _make_df(seed=0), 'two': _make_df(seed=1)}
    ...
    elif local == Locals.PARQUET_ROUNDTRIP:
        df = _make_df()
    ...
    elif local == Locals.PARQUET_DICT:
        dfs = {'one': _make_df(seed=0), 'two': _make_df(seed=1)}
    ...
    elif local == Locals.PARQUET_SERIES_COERCION_IN_DICT:
...
if __name__ == '__main__':
    ...
    run_local(local=Locals.EXCEL_SINGLE)
