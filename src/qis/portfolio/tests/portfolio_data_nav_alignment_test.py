"""Regression tests for date-aligned portfolio reporting ratios.

NAV-normalized costs and market-cap participation must divide each report row by the NAV carrying
the same label. These tests cover row permutations, pandas nullable values, malformed NAV/report
axes, aggregate labels, downstream cost modes, and the raw currency-cost bypass.
"""
...
def test_portfolio_data_compute_mcap_participation_aligns_nav_by_date() -> None:
    """Participation follows trade_level times dated exposure divided by market cap."""
    portfolio = _portfolio()
    mcap = pd.DataFrame(100.0, index=DATES, columns=TICKERS)
    expected = pd.DataFrame([[0.001, 0.002], [0.001, 0.001]], index=DATES, columns=TICKERS)

    actual = portfolio.compute_mcap_participation(mcap=mcap, trade_level=1.0)
...
def _mcap_participation(portfolio: qis.PortfolioData) -> pd.DataFrame:
    """Return the market-cap report used by malformed-axis cases."""
    mcap = pd.DataFrame(100.0, index=DATES, columns=TICKERS)
    return portfolio.compute_mcap_participation(mcap, trade_level=1.0)


@pytest.mark.parametrize(
    "operation",
    [
        pytest.param(_normalized_costs, id="costs"),
        pytest.param(_mcap_participation, id="mcap-participation"),
    ],
)
@pytest.mark.parametrize("case", ["missing", "extra", "different", "duplicate"])
def test_portfolio_data_nav_normalized_reports_reject_malformed_nav_labels(
    operation: Callable[[qis.PortfolioData], pd.DataFrame | pd.Series], case: str
) -> None:
...
@pytest.mark.parametrize("operation", ["costs", "mcap-participation"])
def test_portfolio_data_nav_normalized_reports_reject_duplicate_report_labels(
    operation: str,
) -> None:
    ...
    if operation == "costs":
        ...
        portfolio.prices.index = duplicate_dates
        portfolio.units.index = duplicate_dates
    original_nav = portfolio.nav.copy(deep=True)
    original_costs = portfolio.realized_costs.copy(deep=True)
    original_prices = portfolio.prices.copy(deep=True)
    original_units = portfolio.units.copy(deep=True)
    ...
    with pytest.raises(ValueError, match="report index"):
        if operation == "costs":
            ...
            portfolio.compute_mcap_participation(
                pd.DataFrame(100.0, index=duplicate_dates, columns=TICKERS), trade_level=1.0
            )
