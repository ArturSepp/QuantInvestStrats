...
class PortfolioData:
    ...
    def compute_mcap_participation(self,
        ...
        """Return trade participation as dated portfolio exposure divided by market cap.

        Args:
            mcap: Market capitalization by date and instrument.
            trade_level: Currency notional used to scale portfolio exposure.

        Returns:
            Market-cap participation on the exposure grid.

        Raises:
            ValueError: If exposure and NAV do not have identical unique row labels.
        """
        exposure = self.units.multiply(self.prices)
        nav = self._get_nav_for_report_index(exposure.index, "market-cap participation")
        exposure = exposure.divide(nav, axis=0)
