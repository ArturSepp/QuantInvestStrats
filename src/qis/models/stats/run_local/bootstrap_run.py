...
# internal
from qis.models.bootstrap.bootstrap_numba import (bootstrap_data,
...
def run_local(local: Locals):
    ...
    elif local == Locals.PLOT_PRICE_BOOTSTRAP:
        plot_price_bootrstrap(prices['SPY'])

    elif local == Locals.PLOT_EW_INDEX:
        plot_ew_index_bootrstrap(prices)

    elif local == Locals.FIT_AR_MODEL:
    ...
    elif local == Locals.AR_RESIDUALS:
    ...
    elif local == Locals.PLOT_AR_BOOTSTRAP:
        prices, fund_data = get_test_data()
        data = fund_data.iloc[:, 1]
        plot_ar_bootstrap(data=data)

    elif local == Locals.PLOT_JOINT_BOOTSTRAP:
...
if __name__ == '__main__':
    run_local(local=Locals.FIT_AR_MODEL)
