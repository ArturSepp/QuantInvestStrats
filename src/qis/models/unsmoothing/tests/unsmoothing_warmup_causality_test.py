...
def test_min_obs_for_ar_unsmoothing_matches_causal_availability(
    ...
    assert min_obs_for_ar_unsmoothing(
        ar_order, warmup_period, non_negative=non_negative
    ) == expected_floor
...
def test_non_negative_no_warmup_is_not_rejected_by_tensor_floor() -> None:
    """NNLS with no explicit warmup must use its own shorter availability floor."""
    returns = _seeded_ar_returns(num_periods=4)

    unsmoothed = _adjust_with_diagnostics(
        returns,
        ar_order=2,
        mean_adj_type=MeanAdjType.NONE,
        warmup_period=None,
    ...
    assert unsmoothed.dropna().shape[0] == 1
