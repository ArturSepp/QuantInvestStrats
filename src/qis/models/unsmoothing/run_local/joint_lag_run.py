...
the ``run_local(local=...)`` idiom used for the other unsmoothing engines.
...
def run_local(local: Locals) -> None:
    """Run the selected joint-lag unsmoothing diagnostic."""

    if local == Locals.COEFFICIENT_RECOVERY:
    ...
    elif local == Locals.INVERSION_CONSISTENCY:
    ...
    elif local == Locals.NO_SMOOTHING:
        ...
        returns = (
            0.6 * factor + pd.Series(rng.normal(0.01, 0.05, n), index=idx)
        ).rename('asset').to_frame()
        corrected, phi1, beta1 = adjust_returns_with_joint_unsmoothing(
            returns, factor, span=120, return_diagnostics=True)
        ...
        raise ValueError(f"unknown local diagnostic: {local!r}")

    print(f"{local.name}: PASS")


if __name__ == '__main__':
    run_local(local=Locals.COEFFICIENT_RECOVERY)
