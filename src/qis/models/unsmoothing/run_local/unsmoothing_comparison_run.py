"""
Development comparison of the two unsmoothing methods (EWMA AR(1) vs static GLM) on:
...
Run standalone:
    python -m qis.models.unsmoothing.run_local.unsmoothing_comparison_run
"""
import numpy as np
import pandas as pd
...
def _run_synthetic_constant_smoothing():
...
def _run_synthetic_regime_change():
    ...
    print("  (first-half theta should give higher AR(1) beta; EWMA should show this)")


def _run_cross_method_agreement_liquid():
...
def _run_real_data_smoothed_hf_proxy():
    ...
    print_metrics_table(metrics)
    print(
        f"\nGLM detected smoothing severity (vol inflation): "
        f"{glm_diag.vol_inflation_factor:.2f}x"
    )


def _run_vol_inflation_comparison():
...
def run_local(local: Locals) -> None:
    """Run the selected unsmoothing comparison."""
    pd.set_option('display.max_rows', 500)
    pd.set_option('display.max_columns', 500)
    pd.set_option('display.width', 1000)

    if local == Locals.SYNTHETIC_CONSTANT_SMOOTHING:
        _run_synthetic_constant_smoothing()
    elif local == Locals.SYNTHETIC_REGIME_CHANGE:
        _run_synthetic_regime_change()
    elif local == Locals.CROSS_METHOD_AGREEMENT_LIQUID:
        _run_cross_method_agreement_liquid()
    elif local == Locals.REAL_DATA_ON_SMOOTHED_HF:
        _run_real_data_smoothed_hf_proxy()
    elif local == Locals.VOL_INFLATION_COMPARISON:
        _run_vol_inflation_comparison()
    else:
        raise ValueError(f"unsupported local diagnostic: {local}")


if __name__ == '__main__':
    run_local(local=Locals.SYNTHETIC_CONSTANT_SMOOTHING)
