...
from qis.models.linear.ra_returns import SignalMapType, map_signal_to_weight


class Locals(Enum):
    """Available signal-mapping visual diagnostics."""

    ILLUSTRATION = 1
    COMPARISON = 2
    PROPERTIES = 3
    PROPERTIES_WITH_TAILS = 4
...
def run_local(local: Locals) -> None:
    """Render the selected signal-mapping diagnostic."""
    if local == Locals.ILLUSTRATION:
        signal_mapping_illustration()
    elif local == Locals.COMPARISON:
        signal_mapping_comparison()
    elif local == Locals.PROPERTIES:
        signal_mapping_properties()
    elif local == Locals.PROPERTIES_WITH_TAILS:
        signal_mapping_properties2()
    else:
...
if __name__ == '__main__':
    run_local(local=Locals.ILLUSTRATION)
