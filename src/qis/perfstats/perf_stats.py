...
def resolve_benchmark_source(prices: pd.DataFrame,
    ...
    # ``Index.__eq__`` can return a nullable BooleanArray when another column label is
    # ``pd.NA``; counting that array asks for the truth value of NA and raises. Restrict
    # equality to string labels because ``name`` has already been validated as a string.
    matches = sum(isinstance(label, str) and label == name for label in prices.columns)
    if matches > 1:
        raise ValueError(
