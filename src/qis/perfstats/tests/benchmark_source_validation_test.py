...
def test_resolver_ignores_nullable_unselected_column_label() -> None:
    """A missing unrelated label must not make selected-column counting ambiguous."""
    prices = _benchmark_panel()
    prices.columns = pd.Index([_BENCHMARK_NAME, pd.NA], dtype="string")

    resolved, resolved_name = _PERF_STATS.resolve_benchmark_source(
        prices=prices,
        benchmark=_BENCHMARK_NAME,
        benchmark_price=None,
    )

    assert resolved is prices
    assert resolved_name == _BENCHMARK_NAME
