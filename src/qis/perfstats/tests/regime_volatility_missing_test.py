...
# qis
import qis.perfstats.regime_classifier as regime_classifier
...
def test_estimate_vol_uses_only_finite_values_and_preserves_scalar_type() -> None:
    """Ignore infinities without changing the public one-dimensional scalar type."""
    short_values = np.array([3.0, 4.0, np.nan, np.inf, -np.inf])
    long_values = np.array([1.0, 3.0] + [np.nan] * 16 + [np.inf, -np.inf])
    panel = np.column_stack((short_values, np.full(short_values.shape, np.inf)))

    with warnings.catch_warnings():
        warnings.simplefilter("error")
        short_vol = estimate_vol(short_values)
        long_vol = estimate_vol(long_values)
        panel_vol = estimate_vol(panel)
...
def test_benchmark_vols_quantiles_regime_rejects_infinite_volatility(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Treat infinite volatility estimates as unavailable before quantile classification."""
    prices = _ragged_usable_panel()
    infinite_vols = pd.Series(
        (np.inf, -np.inf),
        index=pd.DatetimeIndex(("2022-01-31", "2022-02-28")),
        name=_BENCHMARK,
    )
    monkeypatch.setattr(
        regime_classifier.ret,
        "compute_sampled_vols",
        lambda **_: infinite_vols,
    )
    classifier = BenchmarkVolsQuantilesRegime(freq="ME", q=_NUM_BUCKETS)

    with pytest.raises(ValueError, match="has no finite volatility observations"):
...
def test_benchmark_vols_quantiles_regime_normalizes_stringified_missing_ids(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Restore missing regime IDs when older pandas stringifies categorical missing values."""
    original_add_quantiles = regime_classifier.dfc.add_quantile_classification

    def add_quantiles_with_string_missing(*args, **kwargs):
        classified, labels = original_add_quantiles(*args, **kwargs)
        hue_name = kwargs["hue_name"]
        x_column = kwargs["x_column"]
        classified.loc[classified[x_column].isna(), hue_name] = "nan"
        return classified, labels

    monkeypatch.setattr(
        regime_classifier.dfc,
        "add_quantile_classification",
        add_quantiles_with_string_missing,
    )
    classifier = BenchmarkVolsQuantilesRegime(freq="ME", q=_NUM_BUCKETS)
    ...
    assert classified["regime"].isna().tolist() == [True] * 4 + [False] * 33
