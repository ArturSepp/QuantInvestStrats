"""Development runner extracted from ``qis.perfstats.tests.perf_stats_test``."""
...
def run_local(local: Locals):
    ...
    elif local == Locals.DRAWDOWN:
        dd_data = compute_rolling_drawdowns(prices=prices['SPY'])
        print(dd_data)

    elif local == Locals.DRAWDOWN_STATS_TABLE:
    ...
    elif local == Locals.RA_PERF_TABLE_NO_SUFFIX_COLUMNS:
    ...
    elif local == Locals.BENCHMARK_RESOLUTION_THREE_CASES:
    ...
    elif local == Locals.DESC_FREQ_TABLE_MEDIAN_FIX:
    ...
    elif local == Locals.DRAWDOWN_STATS_IS_RECOVERED:
    ...
    elif local == Locals.DOWNSIDE_VOL_EDGE_CASES:
...
if __name__ == "__main__":
    run_local(local=Locals.DRAWDOWN_STATS_TABLE)
