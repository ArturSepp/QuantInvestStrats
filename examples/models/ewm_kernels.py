"""
Numba-accelerated EWM kernels: timing, correctness, and covariance tensor.

Benchmarks ``qis.models.ewm_recursion`` and ``qis.compute_ewm`` against
``pandas.ewm`` on large random panels, compares EWMA vol estimators
with different mean-adjustment options, and verifies the
``compute_ewm_covar_tensor`` implementation.
"""
import functools
import timeit
import numpy as np
