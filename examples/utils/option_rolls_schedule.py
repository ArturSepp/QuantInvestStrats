"""
Generate option / futures roll calendars via
``qis.generate_fixed_maturity_rolls``.

Demonstrates weekly (W-FRI) and monthly (last-Friday) roll schedules,
with ``min_days_to_next_roll`` controlling when to switch to the next
contract. The returned DateTimeIndex maps observation timestamps to
their corresponding roll expiry.
"""
import pandas as pd
