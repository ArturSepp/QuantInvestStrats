...
`src/qis/tests/price_data_test.py` (`load_etf_data`) reads a local csv that is **not** distributed
...
`src/qis/market_data/tests/fx_rates_data_test.py` calls `create_fx_rates_data`, which is not
