...
`ruff check qis/` reports about 2,480 violations on a clean checkout, so the lint is red on
arrival and cannot gate anything until that is triaged. About 230 of them are `I001`, which is
not fixable while the house import convention stands: the convention groups stdlib imports
under `# packages` *after* numpy/pandas, and isort wants stdlib first. Either drop `"I"` from
`[tool.ruff.lint] select` or drop the convention — they cannot both hold.
